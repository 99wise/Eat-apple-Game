...
c.execute("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT, rectime text, regdate text)")
...
while True:
 ...
 #플레이어가 음식과 충돌하는지, 열매를 먹는지 검사합니다.
    ...
    if game_over:
        ...
        c.execute("INSERT INTO users (rectime, regdate) VALUES(?,?)",(et, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        c.close()
